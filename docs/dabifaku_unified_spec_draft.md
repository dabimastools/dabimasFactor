# ダビふぁく統合版 実装仕様書（v2 コード照合済み）

## 0. このドキュメントの位置づけ

本ドキュメントは「ダビふぁく統合版」の実装仕様である。

v1 ドラフトの「未確定」「要コード確認」事項を、現行コード（`index.html` 約 6,366 行、`vue/CombinationDialog.js`、`vue/combinationDB.js`、`service-worker.js`）と照合して解消し、実装者がそのまま着手できるレベルまで具体化した。
...
| IndexedDB | 新 DB `dabifaku_unified` | 新 DB `dabifaku_unified`（既存 `DabifacCombinationDB` には触れない） | 既存 DB は version=2 が 3 箇所にハードコードされており、version を上げると全箇所の同時修正が必要になる（§5.5） |
...
- 既存のダビふぁく本体 UI は改変しない。本体側コードへの変更は §12.4 に列挙した「dirty 通知 1 行の追加」と、画面切替用の `v-show` ラップのみ許可する。
- 新規ファイルは `docs/index-component-logic-split-plan.md`（以下「分割計画」）の規約に従う：IIFE + `window.Dabimas` 名前空間、classic script 読み込み、UTF-8 BOM なし。
- 本仕様の実装は、分割計画の Phase 1（定数・因子ロジック外部化）および `logic/storage/` の外部化が完了した後に着手することを推奨する（必須依存は §20 参照）。
...
| ID 生成 | `crypto.randomUUID()` | 現行コードで使用実績あり（`index.html` 5191 行付近） |
...
本章は実装者が前提とすべき「現状の事実」である。

### 4.1 アプリ構成

- 単一ページ。`index.html` 内の `new Vue({ el: "#app" })` が root app で、血統表・ヘッダー・検索・ダイアログすべてを持つ。
- 外部化済み: `vue/CombinationDialog.js`（配合保存ダイアログ）、`vue/factor-dialog.js`（因子選択ダイアログ）。いずれも classic script。
- `vue/combinationDB.js` は ES module（`export`）だが **`index.html` からは読み込まれていない**。IndexedDB の open 処理は CombinationDialog.js 内のインライン `openDB()` と、root app の `ensureCustomHorseDb()` に重複実装されている。
...
| version | **2**（3 箇所にハードコード: `combinationDB.js`、`CombinationDialog.js` の `openDB()`、`index.html` の `ensureCustomHorseDb()`） |
| store | `configs`（配合保存: `{ id: autoIncrement, title, savedAt, configData }`）、`customHorses`（自家製馬: keyPath `id`） |

`configData` は §4.3 の 6 キーの**生文字列**を束ねたもの（＋参照する自家製馬レコードの同梱）。つまり「配合スナップショット」の形式が既に存在する。
...
- 配合復元: `applySavedCombination(record)` が snapshot（`factor` / `factorCategory` / `memo` / `memoStallion` / `memoBroodmare` / `manualInbreed`）を `setOrRemoveLocalStorage()` で localStorage に書き戻し、`refreshLocalDataFromStorage()` を呼ぶ。**この経路が「任意の snapshot を本体に注入する」機能そのもの**であり、作業枠切替はこれを再利用する。
...
- `service-worker.js` は `CACHE_NAME` bump 方式。js/css は cache-first（初回アクセスで runtime cache）、`json/` は network-first。**新規 JS/CSS ファイルを追加したら `urlsToCache` への追加と `CACHE_NAME` の bump が必要**（追加しなくてもオンライン初回アクセスで runtime cache されるが、install 時オフライン完全性のため precache に含める）。

### 4.7 モバイル高さ調整

`applyMobileViewportLayout()` は `this.$refs.appHeader`（`<header ref="appHeader">`）の実高さを `getBoundingClientRect()` で測り、血統表の行高さを画面内に収める。**作業枠タブバーをこの `<header>` 要素の内側に置けば、高さ計算に自動的に含まれ、追加対応は不要**（§9.4）。
...
- **切替（switch-in）**: 選択した作業枠の `snapshot` を localStorage の 6 キーへ書き込み（`setOrRemoveLocalStorage()` 相当）、`refreshLocalDataFromStorage()` で本体を再構築する。既存の配合復元経路（§4.5）と同一処理。
...
(2) appMeta.lastOpenedWorkspaceId と category.lastActiveWorkspaceId を更新
  ↓
...
`DabifacCombinationDB` は version=2 が 3 ファイルにハードコードされており、version 3 に上げると全箇所を同時修正しないと `VersionError` で開けなくなる（各ファイルのコメントにも同旨の警告あり）。統合版は独立した新 DB `dabifaku_unified` を作り、既存 DB・既存コードに一切触れない。
...
| snapshot | localStorage 6 キーの生文字列を束ねたオブジェクト（§12.2）。既存の配合保存 `configData` と同形 |
...
| 内部 ID | `crypto.randomUUID()` で発行する不変 ID |
...
root app に `currentScreen: "home" | "category"` を追加し、`v-if` / `v-show` で切り替える。
...
```text
カテゴリ作成（id = crypto.randomUUID(), sortOrder = 既存最大 + 1）
  ↓
...
キー名は既存 `applySavedCombination()` が受け取る snapshot と同一にしてあり、切替時の注入処理を共通化できる。
...
| メソッド | 対象キー |
|---|---|
| `persistSelectedToStorage()` | factor / factorCategory |
| `memoChange()` | memo |
| `memoChangeStallion()` | memoStallion |
| `memoChangeBroodmare()` | memoBroodmare |
| `persistManualInbreedState()` | manualInbreed |
| `clearManualInbreedForIndex()` | manualInbreed |
| `initializer()` | 全キー（クリア） |
| `onCombinationRestore()` | 全キー（復元） |
...
- 内部 ID は `crypto.randomUUID()`（現行コードで使用実績あり。対象ブラウザは iOS Safari 15.4+ / モダンブラウザで、現行アプリが既に同 API に依存しているため代替実装は不要）。
- 表示番号は保存しない（§11.4）。
...
```text
root app（index.html / 将来は vue/app/*.js）
...
分割計画とは独立に実装可能だが、`logic/storage/` の規約（IIFE / 名前空間）だけは先に揃える。
...
8. **検証**: §21 受入条件、PC・スマホ実機（特に iPhone PWA の pagehide flush）
