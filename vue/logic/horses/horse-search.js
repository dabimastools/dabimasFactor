 ...
 * - IME（日本語入力）のイベント処理。あちらは common-autocomplete 側の役割で、
 *   ここでは「確定済みの検索文字列」を受け取って絞り込むだけ。
 *
 * 分けている理由:
 * - common-autocomplete が「PC入力」「スマホ入力」「メモ入力」「検索ロジック」を
 *   1つのコンポーネントで抱えており、検索文字列の正規化・絞り込みだけでも
 *   先に外へ出すことで、この部分は単体で挙動を確認できるようになる。
 */
(function (window) {
