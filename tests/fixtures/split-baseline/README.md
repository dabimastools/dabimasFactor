...
4. **`localStorageFingerprint.factor.hash`（`dabimasFactor` の FNV-1a）は同一の選択手順を再実行しても一致しない。**
   `selected` 配列の各エントリが持つ `uuid` は選択のたびにランダム生成され、`dabimasFactor` へ保存されるシリアライズ内容に含まれる。そのため `length`（S1 なら 8981）は再現するが `hash` はセッションをまたぐ再実行のたびに変わりうる。Phase 4-12 の回帰確認中に発見。以降のフェーズで `factor.hash` を突き合わせる際は `length` の一致と `uuid` 以外のフィールド（`factorCd` / `inbreedFactorNumtoString` / `categoryNumtoString` / `sameNameGroups` 等）の完全一致をもって代替根拠とし、`hash` 自体の不一致は分割による回帰とみなさない。
